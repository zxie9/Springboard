...
SELECT concat(m1.surname,',',m1.firstname) AS member, concat(m2.surname,',',m2.firstname) AS recommendedby
FROM Members AS m1
...
SELECT name, EXTRACT(MONTH FROM starttime) as month, COUNT(memid) AS member_usage
FROM Bookings
INNER JOIN Facilities
USING (facid)
WHERE memid != 0
